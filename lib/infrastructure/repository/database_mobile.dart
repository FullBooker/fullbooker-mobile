...
class DatabaseMobile<T extends DatabaseExecutor> implements DatabaseBase<T> {
  ...
  Future<bool> isDatabaseEmpty() async {
    // Sum of records from all tables
    final int records = await Future.wait(
      Tables.values.map(
        (Tables t) => countTableRecords(t),
      ),
    ).then(
      (List<int> counts) =>
          counts.reduce((int value, int element) => value + element),
    );
